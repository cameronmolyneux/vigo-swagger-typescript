...
export const isArray = (property: Schema): property is Schema & { items: Schema } => {
  return property.type === DataType.array;
};

export const isObject = (schema: Schema): schema is Schema & { type: 'object' } => {
  return schema.type === DataType.object || schema.properties !== undefined;
};

export const isAllOf = (schema: Schema): schema is Schema & { allOf: Schema[] } => {
  return schema.allOf !== undefined;
};

export const isOneOf = (schema: Schema): schema is Schema & { oneOf: Schema[] } => {
  return schema.oneOf !== undefined;
};

export const isAnyOf = (schema: Schema): schema is Schema & { anyOf: Schema[] } => {
  return schema.anyOf !== undefined;
};

export const isReferenceObject = (schema: Schema): schema is Schema & { $ref: string } => {
